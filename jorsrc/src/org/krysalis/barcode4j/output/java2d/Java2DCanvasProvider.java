 ...
 * @version $Id: Java2DCanvasProvider.java,v 1.3 2008/04/26 20:58:50 admin Exp $
 */
public class Java2DCanvasProvider extends AbstractCanvasProvider {
    ...
    /** @see org.krysalis.barcode4j.output.CanvasProvider */
    public void deviceJustifiedText(String text, double x1, double x2, double y1, String fontName,
        double fontSize) {
     ...
     */
    public void deviceCenteredText(String text, double x1, double x2, double y1, String fontName,
        double fontSize, boolean justify) {
        if (DEBUG) {
            System.out.println("deviceText " + x1 + " " + x2 + " " + (x2 - x1) + " " + y1 + " " +
                text);
            System.out.println("fontSize: " + fontSize + "mm (" + UnitConv.mm2pt(fontSize) + "pt)");
        }
        ...
        final float xxx = font.getStringBounds(text, frc).getBounds().width;
        ...
        if (DEBUG) {
            System.out.println(gv.getLogicalBounds() + " " + gv.getVisualBounds());
        ...
        if (g2d instanceof DrawGlyphVectorDisabled) {
            g2d.drawString(text, (float) (x1 + indent), (float) (y1));
        } else {
        ...
        if (DEBUG) {
            ...
            g2d.draw(new Rectangle2D.Double(x1, y1 - fontSize, x2 - x1, fontSize));
            g2d.draw(new Rectangle2D.Double(x1 + indent, y1 - fontSize, textwidth, fontSize));
        }
    }
}
